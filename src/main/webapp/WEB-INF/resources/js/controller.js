...
cartApp.controller("cartCtrl", function ($scope, $http) {
    ...
    $scope.update_quantity = function (productId) {
        var quantity = $("#update-quantity-"+productId).val();
        var productInfo = $("#product-info-" + productId);
        var stock = productInfo.attr("stock");

        if(stock >= quantity && quantity >= 1){
            $("#quantity-error-cart-"+productId).text();
            $http.put('/rest/cart/edit/' + productId +"?q=" + quantity).success(function () {
                $scope.refreshCart();
            });
            $("#selector-quantity-"+productId).show();
            $("#update-quantity-div-"+productId).hide();
        }else{
            $("#quantity-error-cart-"+productId).text("Only " + stock + " in Stock!");
        }
    };

    $scope.selected_quantity = function (productId) {
        if (this.item.selectedOption == "10+"){
            $("#selector-quantity-"+productId).hide();
            $("#update-quantity-div-"+productId).show();
        }else{
    ...
    $scope.addToCart = function (productId) {
        ...
        $http.put('/rest/cart/add/' + productId +"?q=" + quantity).success(function () {
